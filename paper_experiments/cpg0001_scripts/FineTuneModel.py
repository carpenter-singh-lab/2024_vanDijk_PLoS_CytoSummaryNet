...
for tr_fraction in tr_fraction_list:
    save_name_extension = (
        "finetuned"  # extension of the saved model, specify architecture used
    )
    ...
    optimizer.load_state_dict(
        torch.load(f"{dir_name}/general_ckpt_simpleMLP_V1")["optimizer_state_dict"]
    )
    ...
    for e in range(epochs):
        ...
        for idx, platepoints in enumerate(
            tqdm(
                zip(
                    TrainLoaders[0],
                    TrainLoaders[1],
            ...
            points, labels = (
                [x.to(device) for x in points],
                [y.to(device) for y in labels],
            )
