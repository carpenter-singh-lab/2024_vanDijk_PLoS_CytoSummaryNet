...
def extract_feature_names(df_input):
    ...
    cp_features_analysis = list(
        set(cp_features) - set(locFeature2beremoved) - set(corFeature2beremoved)
    )
...
def handle_nans(df_input):
    ...
    cols2remove_manyNulls = [
        ...
        if (df_input[i].isnull().sum(axis=0) / df_input.shape[0]) > null_vals_ratio
    ]
    cols2remove_lowVars = (
...
def readSingleCellData_r(fileName):
    ...
    cols2remove = [
        i
        for i in pd_df.columns.tolist()
        if ((pd_df[i] == "nan").sum(axis=0) / pd_df.shape[0]) > 0.05
    ]
    print(cols2remove)
...
def edgeCellFilter(df_1):
    ...
    borderLength = int(
        np.percentile(dfWithWTlabels.Cells_AreaShape_MajorAxisLength.values, 90) / 2
    )
    df_1_centCells = df_1.loc[
        ~(
            (df_1["Nuclei_Location_Center_X"] > (imgSize - borderLength))
...
def readSingleCellData_sqlalch_random_image_subset(fileName, n_rand_ims):
    ...
    for compartment in compartments:
        #         compartment_query = "select * from {}".format(compartment)
        compartment_query = "select * from {} WHERE {} IN {};".format(
            compartment, "ImageNumber", list_str
        )
        plateDf_list.append(pd.read_sql(sql=compartment_query, con=conn))
...
def readSingleCellData_sqlalch_concat(fileName):
    ...
    for c in compartments:
        ...
        listOfTandI0 = (
            compartTableDf0["TableNumber"]
            + "_"
            + compartTableDf0["ImageNumber"].astype(str)
            + "_"
            + compartTableDf0["ObjectNumber"].astype(str)
        ).tolist()
        if c == compartments[0]:
    ...
    intersectionOfTableNs = pd.DataFrame(
    ...
    for c in compartments:
        ...
        plateDf0["TableImage"] = (
            plateDf0["TableNumber"]
            + "_"
            + plateDf0["ImageNumber"].astype(str)
            + "_"
            + plateDf0["ObjectNumber"].astype(str)
        )
        plateDf0 = (
            plateDf0[plateDf0["TableImage"].isin(listOfTandI)]
            .sort_values(["TableNumber", "ImageNumber", "ObjectNumber"])
            .reset_index(drop=True)
        )
        ...
        if c == compartments[0]:
            ...
            checkTanIdf = (
                plateDf0[["TableNumber", "ImageNumber", "ObjectNumber"]]
                .sort_values(["TableNumber", "ImageNumber", "ObjectNumber"])
                .reset_index(drop=True)
            )
        else:
            if plateDf0[["TableNumber", "ImageNumber", "ObjectNumber"]].equals(
                checkTanIdf
            ):
                plateDfList.append(
                    plateDf0[
                        list(
                            set(plateDf0.columns)
                            - set(["TableNumber", "ImageNumber", "ObjectNumber"])
                        )
                    ]
                )
            else:
...
def readSingleCellData_sqlalch_well_subset(fileName, wells, meta_well_col_str):
    ...
    compartment_query = "select * from {} WHERE {} IN {};".format(
        "Image", meta_well_col_str, list_str
    )
    start1 = time.time()
    plateImageDf = pd.read_sql(sql=compartment_query, con=conn)
    ...
    for compartment in compartments:
        #         compartment_query = "select * from {}".format(compartment)
        compartment_query = "select * from {} WHERE {} IN {};".format(
            compartment, "ImageNumber", list_str2
        )
...
def readSingleCellData_sqlalch_wellAndObject_subset(fileName, wells, n_rand_objs):
    ...
    for compartment in compartments:
        #         compartment_query = "select * from {}".format(compartment)
        compartment_query = "select * from {} WHERE {} IN {} AND {} IN {};".format(
            compartment, "ImageNumber", list_str2, "ObjectNumber", list_obj
        )
        #         compartment_query = "select * from {} WHERE {} IN {};".format(compartment,"Metadata_Well",list_str)
        plateDf_list.append(pd.read_sql(sql=compartment_query, con=conn))
...
def readSingleCellData_sqlalch_FeatureAndWell_subset(
    fileName, selected_feature, wells, compartments
):
    start1 = time.time()
    # selected_feature='Cells_RadialDistribution_MeanFrac_mito_tubeness_16of16'
    #     selected_feature='Cells_Intensity_IntegratedIntensity_DNA'
    ...
    img_query = "select {} from {} WHERE {} IN {};".format(
        "TableNumber, ImageNumber, well", "Image", "well", list_str
    )

    plateImageDf = pd.read_sql(sql=img_query, con=conn)
    # print(plateImageDf.head()) # debug
    img_nums = plateImageDf.ImageNumber.unique().tolist()
    ...
    list_str2 = list_str2[:-1] + ")"
    ###########################
    pathname_column_names = ", PathName_CellOutlines, PathName_IllumAGP, PathName_IllumBrightfield, PathName_IllumDNA, PathName_IllumER, PathName_IllumMito, PathName_IllumRNA, PathName_NucleiOutlines, PathName_OrigAGP, PathName_OrigBrightfield, PathName_OrigDNA, PathName_OrigER, PathName_OrigMito, PathName_OrigRNA"
    filename_column_names = ", FileName_OrigAGP, FileName_OrigBrightfield, FileName_OrigDNA, FileName_OrigER, FileName_OrigMito, FileName_OrigRNA"
    other_column_names = ", Width_OrigDNA"
    query_cols = (
        "TableNumber, ImageNumber, Metadata_Plate, "
        + selected_feature
        + filename_column_names
        + pathname_column_names
        + other_column_names
    )  # +", "+f2
    compartment_query = "select {} from {} WHERE {} IN {};".format(
        query_cols, compartments, "ImageNumber", list_str2
    )
    plateDf = pd.read_sql(sql=compartment_query, con=conn)
...
def readSingleCellData_sql_CustomFile(fileName, query_file):
    # fileName: sql file
    ...
    plateDfwMeta = pd.read_sql_query(query.read(), conn)
    query.close()
