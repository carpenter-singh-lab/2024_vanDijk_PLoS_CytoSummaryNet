...
if __name__ == "__main__":
    ...
    if plot_distributions:
        ...
        for ax, (title, dependency) in zip(axs.flat, ELLIPSOIDS.items()):
            ...
            if spherize:
                ZCA = utils.ZCA(regularization=regularization, retain_variance=rv).fit(
                    data
                )
    ...
    if roll_dims:
        ...
        for roll_nr in tqdm(range(1324 - 2)):
            ...
            for i, dependency in ELLIPSOIDS.items():
                x, y = get_correlated_dataset(
                    nr_points_per_sample * nr_samples_per_class,
                    dependency,
                    mean,
                    input_dim,
                )
                ...
                if spherize:
                    ZCA = utils.ZCA(
                        regularization=regularization, retain_variance=rv
                    ).fit(data)
                    data = ZCA.transform(data)
                    x, y = data[:, 0], data[:, 1]
                ...
                for index in range(0, len(x), nr_points_per_sample):
                    x_set, y_set = (
                        x[index : index + nr_points_per_sample],
                        y[index : index + nr_points_per_sample],
                    )
                    # if int(i) == 0 or int(i) == 4 or int(i) == 5 or int(i) == 7:
                    if fill_to_model_input_dim:
                        s = np.column_stack(
                            [x_set, y_set, np.zeros((x_set.shape[0], 1324 - input_dim))]
                        )
                        s = np.roll(s, roll_nr)
                        val_samples.append(
                            {"data": s, "cov_matrix": dependency, "label": label_idx}
                        )

                    else:
                        val_samples.append(
                            {
                                "data": np.column_stack([x_set, y_set]),
            ...
            if train_model:
                ...
                for e in range(epochs):
                    ...
                    for idx in range(0, len(train_samples), bs):
                        points = torch.stack(
                            [
                                ...
                                for x in train_samples[idx : idx + bs]
                            ]
                        )
                        labels = torch.stack(
                            [
                                torch.tensor(x["label"], dtype=torch.int16)
                                for x in train_samples[idx : idx + bs]
                            ]
                        )
                    ...
                    with torch.no_grad():
                        for idx in range(0, len(val_samples), bs):
                            points = torch.stack(
                                [
                                    torch.tensor(x["data"], dtype=torch.float32)
                                    for x in val_samples[idx : idx + bs]
                                ]
                            )
                            labels = torch.stack(
                                [
                    ...
                    print(
                        f"Epoch {e}. Training loss: {tr_loss}. Validation loss: {val_loss}."
                    )
            ...
            with torch.no_grad():
                for idx in range(len(val_samples)):
                    points = torch.unsqueeze(
                        torch.tensor(val_samples[idx]["data"], dtype=torch.float32),
                        dim=0,
                    )
            ...
            BM_profiles.columns = [f"f{x}" for x in range(BM_profiles.shape[1] - 1)] + [
                "Metadata_labels"
            ]
            AP_MLP = utils.CalculateMAP(
                MLP_profiles, "cosine_similarity", groupby="Metadata_labels"
            )
            ...
            AP_BM = utils.CalculateMAP(
                BM_profiles, "cosine_similarity", groupby="Metadata_labels"
            )
