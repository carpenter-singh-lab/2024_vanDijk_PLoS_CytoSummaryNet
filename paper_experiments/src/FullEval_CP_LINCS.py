...
def fulleval(args):
    ...
    percent_matching = (
        args.find_sister_compounds
    )  # If false calculate percent replicating
    ...
    ## Load all data
    rootDir = rf"datasets/{dataset_name}"
    ...
    ValDataset = DataloaderEvalV5(
        Total, remove_columns=remove_columns, remove_noisy_cells=remove_noisy_cells
    )
    ...
    with torch.no_grad():
        for points, labels, filtered_points in tqdm(loader):
            ...
            # Calculate benchmark
            c2 = pd.concat(
                [pd.DataFrame(points.mean(dim=1)), pd.Series(labels)], axis=1
            )
            average_profiles = pd.concat([average_profiles, c2])

            # Calculate filtered benchmark
            c3 = pd.concat(
                [pd.DataFrame(filtered_points.mean(dim=1)), pd.Series(labels)], axis=1
            )
            filtered_average_profiles = pd.concat([filtered_average_profiles, c3])

    ## Rename columns and normalize features
    # Rename feature columns
    MLP_profiles.columns = [f"f{x}" for x in range(MLP_profiles.shape[1] - 1)] + [
        "Metadata_labels"
    ]
    average_profiles.columns = [
        f"Cells_{x}" for x in range(average_profiles.shape[1] - 1)
    ] + ["Metadata_labels"]
    filtered_average_profiles.columns = [
        f"Cells_{x}" for x in range(filtered_average_profiles.shape[1] - 1)
    ] + ["Metadata_labels"]

    MLP_profiles = MLP_profiles[
        MLP_profiles.Metadata_labels.duplicated(keep=False)
    ]  # filter out non-replicates
    average_profiles = average_profiles[
        average_profiles.Metadata_labels.duplicated(keep=False)
    ]  # filter out non-replicates
    filtered_average_profiles = filtered_average_profiles[
        filtered_average_profiles.Metadata_labels.duplicated(keep=False)
    ]  # filter out non-replicates
    ...
    features = feature_select(
        filtered_average_profiles.iloc[:, :-1],
        operation=[
            "variance_threshold",
            "correlation_threshold",
            "drop_na_columns",
    ...
    filtered_average_profiles = pd.concat(
        [features, filtered_average_profiles.iloc[:, -1]], axis=1
    )
    ...
    if save_features_to_csv:
        MLP_profiles.to_csv(
            f"{args.output_path}/{dataset_name}_profiles/MLP_profiles_{args.dose_point}.csv",
            index=False,
        )
        average_profiles.to_csv(
            f"{args.output_path}/{dataset_name}_profiles/average_profiles_{args.dose_point}.csv",
            index=False,
        )
        filtered_average_profiles.to_csv(
            f"{args.output_path}/{dataset_name}_profiles/filtered_average_profiles_{args.dose_point}.csv",
            index=False,
        )
    split = int(MLP_profiles.shape[0] * train_val_split)

    temp_df = bigdf[["Metadata_labels", "moa", "pert_iname"]][
        ~bigdf.Metadata_labels.duplicated(keep="last")
    ]
    ...
    filtered_average_profiles = pd.merge(
        filtered_average_profiles, temp_df, on="Metadata_labels"
    )

    print(
        "Dropping ",
        MLP_profiles.shape[0] - MLP_profiles.dropna().reset_index(drop=True).shape[0],
        "rows due to NaNs",
    )
    MLP_profiles = MLP_profiles.dropna().reset_index(drop=True)
    average_profiles = average_profiles.dropna().reset_index(drop=True)
    filtered_average_profiles = filtered_average_profiles.dropna().reset_index(
        drop=True
    )
    print("New size:", MLP_profiles.shape)
    shuffled_profiles = average_profiles.copy()
    shuffled_profiles.iloc[:, :-3] = pd.DataFrame.from_records(
        np.ones(shuffled_profiles.iloc[:, :-3].shape)
    )

    ## Calculate mean average precision
    ap_mlp = utils.CalculateMAP(
        MLP_profiles,
        "cosine_similarity",
        groupby=mAP_label,
        percent_matching=percent_matching,
    )
    ap_bm = utils.CalculateMAP(
        average_profiles,
        "cosine_similarity",
        groupby=mAP_label,
        percent_matching=percent_matching,
    )
    ap_bm_filtered = utils.CalculateMAP(
        filtered_average_profiles,
        "cosine_similarity",
        groupby=mAP_label,
        percent_matching=percent_matching,
    )
    ap_shuffled = utils.CalculateMAP(
        shuffled_profiles,
        "cosine_similarity",
        groupby=mAP_label,
        percent_matching=percent_matching,
    ...
    try:
        ...
        pass
    ap_mlp.groupby("compound").agg({"AP": "mean", "count": "sum"}).to_csv(
        f"{args.output_path}/{dataset_name}/MLP_mAP_{args.dose_point}_{encoding_label}.csv"
    )
    ap_bm.groupby("compound").agg({"AP": "mean", "count": "sum"}).to_csv(
        f"{args.output_path}/{dataset_name}/average_mAP_{args.dose_point}_{encoding_label}.csv"
    )
    ap_bm_filtered.groupby("compound").agg({"AP": "mean", "count": "sum"}).to_csv(
        f"{args.output_path}/{dataset_name}/filtered_average_mAP_{args.dose_point}_{encoding_label}.csv"
    )

    ap_mlp.to_csv(
        f"{args.output_path}/{dataset_name}/raw_MLP_mAP_{args.dose_point}_{encoding_label}.csv"
    )
    ap_bm.to_csv(
        f"{args.output_path}/{dataset_name}/raw_average_mAP_{args.dose_point}_{encoding_label}.csv"
    )
    ap_bm_filtered.to_csv(
        f"{args.output_path}/{dataset_name}/raw_filtered_average_mAP_{args.dose_point}_{encoding_label}.csv"
    )

    print(
        "Total mean mAP MLP:",
        ap_mlp.AP.mean(),
        "\nTotal mean precision at R MLP:",
        ap_mlp["precision at R"].mean(),
    )
    print(
        ap_mlp.groupby("compound")
        .mean()
        .sort_values("AP")
        .iloc[-30:, :]
        .round(4)
    ...
    print(
        "Total mean mAP BM:",
        ap_bm.AP.mean(),
        "\nTotal mean precision at R BM:",
        ap_bm["precision at R"].mean(),
    )
    print(
        ap_bm.groupby("compound")
        .mean()
        .sort_values("AP")
        .iloc[-30:, :]
        .round(4)
    ...
    print("\n")
    print(
        "Total mean mAP filtered BM:",
        ap_bm_filtered.AP.mean(),
        "\nTotal mean precision at R BM:",
        ap_bm_filtered["precision at R"].mean(),
    )
    print(
        ap_bm_filtered.groupby("compound")
        .mean()
        .sort_values("AP")
        .iloc[-30:, :]
        .round(4)
    ...
    print("\n")
    print(
        "Total mean mAP shuffled:",
        ap_shuffled.AP.mean(),
        "\nTotal mean precision at R shuffled:",
        ap_shuffled["precision at R"].mean(),
    )

    print("\n")
    # Conduct Welch's t-Test and print the result
    print(
        "Welch's t-test between mlp mAP and bm mAP:",
        stats.ttest_ind(np.array(ap_mlp.AP), np.array(ap_bm.AP), equal_var=False),
    )
    print("\n")
    ...
    f.write("\n")
    f.write(
        f"Training samples || mean:{ap_mlp.iloc[:split,1].mean()}\n"
        + ap_mlp.iloc[:split, :]
        .groupby("compound")
        .mean()
    ...
    f.write("\n")
    f.write(
        f"Validation samples || mean:{ap_mlp.iloc[split:,1].mean()}\n"
        + ap_mlp.iloc[split:, :]
        .groupby("compound")
        .mean()
        .sort_values(by="AP", ascending=False)
    ...
    f.write("\n")
    f.write(
        f"Training samples || mean:{ap_bm.iloc[:split, 1].mean()}\n"
        + ap_bm.iloc[:split, :]
        .groupby("compound")
        .mean()
        .sort_values(by="AP", ascending=False)
    ...
    f.write("\n")
    f.write(
        f"Validation samples || mean:{ap_bm.iloc[split:, 1].mean()}\n"
        + ap_bm.iloc[split:, :]
        .groupby("compound")
        .mean()
        .sort_values(by="AP", ascending=False)
    ...
    f.write("\n")
    f.write(
        f"Training samples || mean:{ap_shuffled.iloc[:split, 1].mean()}\n"
        + ap_shuffled.iloc[:split, :]
        .groupby("compound")
        .mean()
        .sort_values(by="AP", ascending=False)
        .to_markdown()
    ...
    f.write("\n")
    f.write(
        f"Validation samples || mean:{ap_shuffled.iloc[split:, 1].mean()}\n"
        + ap_shuffled.iloc[split:, :]
        .groupby("compound")
        .mean()
        .sort_values(by="AP", ascending=False)
    ...
    if percent_matching:
        ...
        sorted_dictionary = {
            k: [v]
            for k, v in sorted(
                average_perturbation_map.items(), key=lambda item: item[1], reverse=True
            )
        }
        ...
        allresultsdf = pd.DataFrame(
            {
                "Training mAP model": [ap_mlp.iloc[:split, 1].mean()],
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate a trained model by calculating mAP for either"
    ...
    # Optional positional argument
    parser.add_argument(
        "dataset_name", nargs="?", const="LINCS", type=str, help="Dataset name"
    )
    # Optional positional argument
    parser.add_argument(
        "model_path",
        nargs="?",
        const="wandb/latest-run/files",
        type=str,
    ...
    parser.add_argument(
        "output_path",
        nargs="?",
        const="FinalModelResults/mAP",
        type=str,
        help="Path to save all output files",
    )
