...
def clean_cells(df):
    ...
    df = df[
        (stats.zscore(df[top_corr_feats]) > -2).all(axis=1)
        & (stats.zscore(df[bot_corr_feats]) < 2).all(axis=1)
    ]
...
class DataloaderEvalV5(Dataset):
    ...
    def __init__(
        self, df, preprocess=False, remove_columns=None, remove_noisy_cells=False
    ):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        # extract numpy array
        if (
            sample1["cell_features"].shape[1] == 1781
            and self.remove_columns is not None
        ):
            if sample1["cell_features"].shape[0] == 1:
                ...
                sample1["cell_features"] = sample1["cell_features"].drop(
                    self.remove_columns, axis=1
                )

        elif (
            sample1["cell_features"].shape[1] == 1781
            and sample1["cell_features"].shape[0] == 1
        ):
        ...
        if self.remove_noisy_cells:
            if isinstance(sample1["denoised_cell_features"], pd.DataFrame):
                sample1["denoised_cell_features"] = sample1[
                    "denoised_cell_features"
                ].to_numpy()
            denoised_features = sample1["denoised_cell_features"][
                ~np.isnan(sample1["denoised_cell_features"]).any(axis=1)
            ]
    ...
    def __getitem__(self, idx):
            ...
            for P in paths:
                with open(P, "rb") as f:
                    ...
                    if s1["cell_features"].shape[0] == 1:
                        continue
                    if (
                        s1["cell_features"].shape[1] == 1781
                        and self.remove_columns is not None
                    ):
                        s1["cell_features"] = s1["cell_features"].drop(
                            self.remove_columns, axis=1
                        )
                    sample.append(s1)
            # Extract numpy array
        ...
        for _ in range(self.nr_sets):
            ...
            if nr_wells <= len(sample):
                which_wells = np.random.choice(
                    len(sample), nr_wells, replace=False
        ...
        sampled_features = np.array(
            sampled_features
        )  # first convert to numpy array for speed
    ...
    def __getitem__(self, idx):
        ...
        if self.groupDF is not None:
            ...
            for P in paths:
                with open(P, "rb") as f:
                    s1 = pickle.load(f)
                    s1["cell_features"] = s1["cell_features"][
                        ~np.isnan(s1["cell_features"]).any(axis=1)
                    ]  # Remove possible NaNs
        ...
        for _ in range(self.nr_sets):
            # Generate random number of wells to combine into one sample
            nr_wells = np.random.randint(
                1, 3, 1
            )  # either 1 or 2 wells combined (note that 2 is not exclusive)
            if nr_wells <= len(sample):
                which_wells = np.random.choice(
                    len(sample), nr_wells, replace=False
                )  # sampling without replacement
