...
class CytoSummaryNet(nn.Module):
    ...
    def __init__(
        ...
        assert reduction in ["sum", "mean", "max"], "Invalid reduction method. Choose from 'sum', 'mean', or 'max'."
    ...
    def _build_layers(
        ...
        if isinstance(num_layers, int):
            ...
            for i in range(num_layers - 1):
                layers.append(nn.Linear(input_dim if i == 0 else intermediate_dim, intermediate_dim))
