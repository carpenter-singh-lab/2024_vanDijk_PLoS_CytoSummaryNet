...
#%%
run = wandb.init(project="FeatureAggregation", mode='dryrun', tags=['Generalized Models'])  # 'dryrun'
...
nr_sets = 5 # number of sets you sample per well
BS = 9  # 54 for grouped // batch size as passed to the model (nr of wells that you sample per batch)
...
plates = ['BR00116625highexp_FS', 'BR00116628highexp_FS', 'BR00116629highexp_FS'] # STAIN4
#plates = ['BR00115134_FS', 'BR00115125_FS', 'BR00115133highexp_FS'] # STAIN3
#plates = ['BR00112201_FS', 'BR00112198_FS', 'BR00112204_FS'] # STAIN2
...
#%% Load model
if load_model:
    run_name = r'run-20220420_102837-200n10dn'
...
for e in range(E, E+epochs):
    ...
    with torch.no_grad():
        for dataloader_idx in tqdm(range(len(ValLoaders))):
            ...
            temp_mAPs.append(AP.AP.mean())

        val_loss = sum(temp_losses) / len(temp_losses)
        val_loss = val_loss.item()
        val_mAP = sum(temp_mAPs) / len(temp_mAPs)

    wandb.log({"Val loss": val_loss, "Val mAP": val_mAP}, step=e)  # send data to wandb.ai
