...
# plt.hist(NRFs, bins=20)
# plt.title(platestring)
# plt.show()
...
print(f'Training samples || mean:{AP.iloc[:288,:].mean()}\n', AP.iloc[:288,:].groupby('compound').mean().sort_values(by='AP',ascending=False).to_markdown())
print(f'Validation samples || mean:{AP.iloc[288:,:].mean()}\n', AP.iloc[288:,:].groupby('compound').mean().sort_values(by='AP',ascending=False).to_markdown())

# WRITE TO FILE
f = open('figures/MAPexp4.txt', 'a')
f.write('\n')
f.write('\n')
...
f.write('\n')
f.write(f'Training samples || mean:{AP.iloc[:288,:].mean()}\n' + AP.iloc[:288,:].groupby('compound').mean().sort_values(by='AP',ascending=False).to_markdown())
f.write('\n')
f.write(f'Validation samples || mean:{AP.iloc[288:,:].mean()}\n' + AP.iloc[288:,:].groupby('compound').mean().sort_values(by='AP',ascending=False).to_markdown())
f.close()
